...
#define BLOCKSIZE 64   //磁盘块大小，单位字节
...
{
    fcb file_fcb;
    char dir[DIRNUM]; // 打开文件所在路径字符串,以便快速检查指定文件是否已经打开
    char topenfile;   //打开表项是否为空, 0:空  1:表示已被占用
    int count;        //读写指针的位置
    char fcbstate;    //文件的FCB是否被修改，如果修改了，则置为1；否则置为0
} useropen;
...
const int fatnum = ceil((double)2 * BLOCKNUM / BLOCKSIZE); //ceil返回最小整数值，每个FAT文件2字节，管理n个磁盘块，存储需要
...
bool addDiritem(unsigned short fd, string str, int type)
{
    ...
    while (length > BLOCKSIZE) //如果大于当前块的容量
    {
        length -= BLOCKSIZE;
...
{
    int i, j;
    myvhard = (unsigned char*)malloc(SIZE); //基地址
    ...
    else
    {
        ...
        memcpy(myvhard, buf, SIZE); //两个指针位置，选择n个字节数
